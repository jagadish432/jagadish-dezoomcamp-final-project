...
start = time()
# All this setup required to access GCS bucket and Bigquery, is only for local/VM machines only, 
# not for the GCP dataproc clusters
credentials_location = '/home/jagadish/.gc/finaldatazoomcamp.json'

conf = SparkConf().setMaster("local[*]").setAppName('generate-stats-local') \
...
ipl_matches.write.partitionBy("season").format("bigquery").mode('Overwrite') \
    ...
    .option("temporaryGcsBucket","jagadish_data_lake_datazoomcamp-jagadish-final") \
